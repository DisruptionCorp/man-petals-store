...
describe('Product Model', () => {
  ...
  it('description is text', () => {
    expect(
      Product.create({
        name: 'Banana',
        description:
          'lots of text, lots of text, lots of text, lots of text, lots of text, lots of text, lots of text, lots of text, lots of text,',
      })
    ).to.be.ok;
  ...
  it('inventory quantity is a number', async () => {
    const cbj = await Product.create({
      name: 'Chicago Bulls Jersey',
      inv_quantity: 30,
    });

    expect(typeof cbj.inv_quantity).to.equal('number');
    expect(Product.create({ name: 'Lakers Jersey', inv_quantity: false })).to.be
      .rejected;
  });
